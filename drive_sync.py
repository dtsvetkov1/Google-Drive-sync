...
APPLICATION_NAME = 'Drive API Python Sync LaLaLa'

GOOGLE_MIME_TYPES = {
    'application/vnd.google-apps.document': 'applicatioapplication/vnd.openxmlformats-officedocument.wordprocessingml.document',
...
def get_tree(folder_name, folder_id, tree_list, root, parents_id, service):
...
def main():
    ...
    get_tree(folder_name, folder_id, tree_list, root, parents_id, service)
    os_tree_list = []
    ...
    # new folders on drive, which you dont have(i suppose heh)
    ...
    for folder_dir in upload_folders:
        ...
        pre_last_dir = folder_dir.split('/')[-2]
        ...
        for f in files:
            some_metadata = {'name': f, 'parents': [folder_id]}
            MIMETYPE = mimetypes.MimeTypes().guess_type(
                os.path.join(variable, f))[0]
            media = MediaFileUpload(os.path.join(variable, f),
                                    mimetype=MIMETYPE)
    ...
    for folder_dir in exact_folders:
        ...
        os_files = [f for f in os.listdir(variable)
                    if os.path.isfile(os.path.join(variable, f))]
        results = service.files().list(
            pageSize=100, q=('%r in parents and \
            mimeType!="application/vnd.google-apps.folder" and \
        ...
        for file in refresh_files:
            '''
                If last modification date of PC file is later
                than the same Drive file,
                then replace file with ne wer version.
            '''   
            ...
            mtime = [f['modifiedTime']
                     for f in items if f['name'] == file['name']][0]
        ...
        for file in remove_files:
            ...
            file_id = [f['id'] for f in items if f['name'] == file['name']][0]
            delete_file = service.files().delete(fileId=file_id).execute()

        # Upload new files on Drive
        for file in upload_files:

            file_dir = os.path.join(variable, file)

            # File's new content.
            filemime = mimetypes.MimeTypes().guess_type(file_dir)[0]
            file_metadata = {'name': file, 'parents': [parents_id[last_dir]]}
            media_body = MediaFileUpload(file_dir, mimetype=filemime)

            updated_file = service.files().create(body=file_metadata,
                                                  media_body=media_body,
                                                  fields='id').execute()
    ...
    for folder_dir in remove_folders:
        ...
        delete_folders = service.files().delete(fileId=folder_id).execute()
